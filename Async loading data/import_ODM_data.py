...
class ODMInstance:
    def __init__(self, params):
        self.session = self.authenticate(params)
        self.template = params.TEMPLATE_ACCESSION or self.get_template(params)

    @staticmethod
    def authenticate(params):
        aut_url = "/frontend/endpoint/application/invoke/genestack/signin/authenticateByApiToken"
        url_authenticate = params.SERVER + aut_url
        session = requests.Session()
        result = session.post(url_authenticate, json=[params.TOKEN])
    ...
    def get_template(self, params):
        url_request_template = (params.SERVER +
                                "/frontend/endpoint/application/invoke/genestack"
                                "/study-metainfotemplateeditor/listTemplates")
        response = self.session.post(url=url_request_template, json=[])
...
def check_access_to_study(params, odm_instance):
    response = odm_instance.get_study(params)
    if response.status_code != 200:
        if params.DEBUG_FLAG:
            print(response, response.text)
        _err("You haven't access permissions to study: %s or it doesn't exist!" % params.study_accession,
             in_red=True)
        sys.exit(1)
...
# pylint: disable-next=too-many-arguments
def _async_import(kind, metadata_link, params, template_id=None, data_link=None,
                  prev_version=None):
    ...
    payload = _prepare_etl_payload(kind, metadata_link, template_id, data_link,
                                   prev_version, params.ETL_SOURCE)
...
def add_study(odm_instance, params):
    ''' Create new study or return existing one '''
    if params.study_accession:
        check_access_to_study(params, odm_instance)
        return params.study_accession
    job_info, study_exists = _async_import('study', metadata_link=params.study_link, template_id=odm_instance.template,
                                           params=params)
...
def add_and_link_samples(odm_instance, params, study, sample_link):
    job_info, samples_exist = _async_import('samples', metadata_link=sample_link,
                                            template_id=odm_instance.template,
                                            params=params)
...
def get_libs_preps_by_study(file_type, params, study):
    ...
    if params.DEBUG_FLAG:
        print(response, response.text)
    _err("You haven't access permissions to study: {} or it doesn't exist!"
...
def add_and_link_libs_preps(samples_group, metadata_link, file_type, params, odm_instance, study, failures):
...
def add_signals(file_signal, file_metadata, file_type, params, odm_instance):
...
def link_by_parent(what, accession_to, accession_from, params):
    ...
    url = '/'.join([params.SERVER, COMMON_URL_PREFIX, INTEGRATION_PREFIX.rstrip('/') % params.APP_VERSION,
                    ENDPOINT_DICT[what]])
...
class SaneArgumentParser(argparse.ArgumentParser):
    ...
    def parse_args(self, args=None, namespace=None):
        if args is None:
            ...
            args = [a.replace('_', '-') if a.startswith('--') else a
                    for a in sys.argv[1:]]
        return super(SaneArgumentParser, self).parse_args(args, namespace)


# pylint: disable-next=protected-access
class StudyAction(argparse._StoreAction):
    """ Ensures that only one study argument is passed """
    def __call__(self, parser_, namespace, values, option_string=None):
        already_defined = any(getattr(namespace, v) is not None
                              for v in ('study_link', 'study_accession'))
        if already_defined:
            raise argparse.ArgumentError(self,
                                         "provide exactly one study via a link "
                                         "or an existing study accession")
...
def add_lib_prep(sample_group, nodes, link_cache, params, odm_instance, study, failures):
    for node in nodes:
        lib_prep_file_type = node['tag']
        url = node['value']
        lib_prep_group = link_cache.get(url, None)
        if lib_prep_group is None:
            lib_prep_group = add_and_link_libs_preps(sample_group, url, lib_prep_file_type, params, odm_instance,
                                                     study, failures)
            link_cache[url] = lib_prep_group
        children = node.get('children', [])
        add_signals_to_parent(lib_prep_group, lib_prep_file_type, children, link_cache, params, odm_instance,
                              failures)


def existing_lib_prop(node, signal_cache, params, odm_instance, failures):
    lib_prep_file_type = node['tag']
    lib_prep_group = node['value']
    children = node.get('children', [])
    add_signals_to_parent(lib_prep_group, lib_prep_file_type, children, signal_cache, params, odm_instance,
                          failures)


def handle_lib_prep_case(parser_state, odm_instance, params, study, failures):
    link_cache = {}
    for sample_node in parser_state.sample_node_list:
        ...
        if value == 'implicit':
            ...
            existing_lib_prop(sub_node, link_cache, params, odm_instance, failures)
            return
        ...
        if is_acc(value):
            ...
            try:
                sample_group = add_and_link_samples(odm_instance, params, study, sample_link=value)
            except GroupLinkingError as ex:
                if not params.IGNORE_LINKING_ERRORS:
                    sys.exit(1)
                failures.append(ex)
        ...
        add_lib_prep(sample_group, libraries_and_preparations, link_cache, params, odm_instance, study, failures)


def handle_samples_signals_case(parser_state, odm_instance, params, study, failures):
    signal_cache = {}
    for sample_node in parser_state.sample_node_list:
        value = sample_node['value']
        ...
        add_signals_to_parent(sample_group, 'sample', signals, signal_cache, params, odm_instance, failures)
...
def main():
    ...
    behaviour_args.add_argument("--fail-if-exists", dest="FAIL_IF_FILE_EXISTS",
                                action='store_true',
                                help=("exit if the file being imported already exists on the server "
                                      "(default is to re-use existing file)"))
    args = parser.parse_args()

    args.SERVER = args.SERVER.rstrip('/')

    if not (args.study_link or args.study_accession):
        _err("No study link or accession was provided", in_red=True)
        sys.exit(1)

    if not args.TOKEN:
        print(red_text("Please provide an API token"))
        sys.exit(1)

    args.headers = {"Genestack-API-Token": args.TOKEN,
                    "Accept": "application/json",
                    "Content-Type": "application/json"}

    failures = []

    check_for_repeated_links(args.study_link, parser_args_state.sample_node_list,
                             links_cache={})
    ...
    if args.LINK_SIGNALS_TO_ALL_SAMPLES:
    ...
    if args.LINK_SIGNALS_TO_ALL_SAMPLES and mapping_files_number > 1:
    ...
    if parser_args_state.has_libraries_or_preparations():
        check_signal_versions_libs_preps(parser_args_state.sample_node_list, args.study_accession)
    else:
        check_signal_versions_samples(parser_args_state.sample_node_list, args.study_accession)

    if args.dump_args_as_json:
        print(json.dumps(parser_args_state.sample_node_list, indent=2))
        sys.exit(0)

    odm_instance = ODMInstance(params=args)
    study = add_study(odm_instance=odm_instance, params=args)
    if parser_args_state.has_libraries_or_preparations():
        handle_lib_prep_case(parser_args_state, odm_instance, args, study, failures)
    else:
        handle_samples_signals_case(parser_args_state, odm_instance, args, study, failures)
